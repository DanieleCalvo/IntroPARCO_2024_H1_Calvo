...
// Transpose a matrix using OpenMP work sharing
void matTransposeWS(float** M, float** T, int N) {
    if (!checkSym(M, N)) {
        // Use OpenMP parallelism with SIMD optimization
...
void init_matrix(float** M, int N) {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            M[i][j] = j + (i * 10); // Example initialization
        }
    }
}
